...
import {
  CalendarDays, CheckCircle2, ChevronLeft, ChevronRight, Info,
  Loader2, MapPin, Minus, Plus, ShieldCheck, Sparkles, Star, Tag,
...
import { getAmenityIcon, SectionCard, InfoRows, Stars } from './components/SharedUI';

// Helpers
import {
  PLACEHOLDER_IMAGE, DEFAULT_AMENITIES,
  formatCurrency, parseNumber, ensureIsoDate,
  normalizeHotelId, deriveHotelId, normalizeAmenities,
  extractPriceCandidate, sumFoodSelections, calculateStayNights,
  deriveRoomAvailability, requiredRoomsForGuests, pickMonthlyOverride,
  badgeForPolicy
...
export default function BookNowPage() {
  ...
  const selectedRoomsPayload = useMemo(() => {
    ...
    return [{
      _id: selectedRoom.roomId, roomId: selectedRoom.roomId, name: selectedRoom.name,
      type: selectedRoom.name, gstPercent: selectedRoom.gstPercent,
      priceWithGST: selectedRoom.priceWithGST, price: effectiveRoomNightlyPrice,
      finalPrice: effectiveRoomNightlyPrice, monthlyPriceApplied: Boolean(monthlyOverride),
      monthlyPriceMeta: monthlyOverride ? { startDate: monthlyOverride.startDate, endDate: monthlyOverride.endDate, monthPrice: monthlyOverride.monthPrice, sourceId: monthlyOverride._id } : undefined
    }];
  ...
  useEffect(() => {
    ...
    setGuestDetails({ name: user?.name || user?.displayName || '', email: user?.email || '', phone: user?.mobile || '' });
  }, [isLoggedIn, user]);
  ...
  const { handleApplyCoupon, handleOfflineBooking, recalculateGst } = useBookingOperations({
    hotelId, hotelData: hotel, user, guestDetails, selectedRooms: selectedRoomsPayload,
    selectedFood, couponCode, roomsCount, guestsCount, checkInDate, checkOutDate,
    finalTotal: priceSummary.netPay, discountPrice, setDiscountPrice, setIsCouponApplied, setGstAmount, toBeCheckRoomNumber: roomsCount
  });
  ...
  const policyHighlights = useMemo(() => {
    ...
    return base.filter((x) => x.val !== undefined && x.val !== null && String(x.val).trim() !== '').slice(0, 5);
  }, [policy0]);
  ...
  const reviewCount = useMemo(() => {
    const direct = parseNumber(hotel?.reviewCount ?? hotel?.reviewsCount ?? hotel?.ratingsCount ?? 0, 0);
    return direct > 0 ? direct : reviewsArray.length;
  }, [hotel, reviewsArray.length]);

  const safeRating = useMemo(() => {
    if (hotelRating !== null && hotelRating !== undefined) return Math.max(0, Math.min(5, hotelRating));
    const fromReviews = reviewsArray.map((r) => parseNumber(r?.rating ?? r?.stars ?? 0, 0)).filter((x) => x > 0 && x <= 5);
    if (!fromReviews.length) return null;
    return Math.round((fromReviews.reduce((a, b) => a + b, 0) / fromReviews.length) * 10) / 10;
  ...
  const amenitiesPreview = useMemo(() => allAmenities.slice(0, AMENITIES_PREVIEW), [allAmenities]);
  const amenitiesRemaining = Math.max(allAmenities.length - AMENITIES_PREVIEW, 0);
    ...
    <div className={compact ? '' : 'bg-white rounded-3xl shadow-lg border border-gray-100 overflow-visible relative z-40'}>
      {!compact && (
        <div className="bg-gradient-to-r from-blue-600 to-blue-700 p-4 text-white rounded-t-3xl">
          ...
          <p className="text-xs text-blue-100">{nights} Night{nights > 1 ? 's' : ''} at {hotelName}</p>
        </div>
      )}
      <div className={`${compact ? '' : 'p-5'} space-y-4`}>
        ...
        <div className="flex gap-3 relative z-50">
          ...
          <div className="relative" ref={roomsPopupRef}>
            <button onClick={() => setShowRoomsPopup(!showRoomsPopup)} className="h-full flex flex-col items-start p-3 bg-gray-50 rounded-2xl hover:bg-gray-100 transition border border-transparent hover:border-blue-200 min-w-[120px]">
            ...
            {showRoomsPopup && <RoomsGuestsPopup rooms={roomsCount} guests={guestsCount} onRoomsChange={setRoomsCount} onGuestsChange={setGuestsCount} onClose={() => setShowRoomsPopup(false)} />}
          </div>
        </div>

        {/* Price Breakdown */}
        <InfoRows rows={[
          { label: `Room Price (${nights} night${nights > 1 ? 's' : ''})`, value: `₹${formatCurrency(priceSummary.roomSubtotal)}` },
          ...(priceSummary.addonsTotal > 0 ? [{ label: 'Meals & Addons', value: `₹${formatCurrency(priceSummary.addonsTotal)}` }] : []),
          { label: 'Taxes & Fees', value: `₹${formatCurrency(priceSummary.taxes)}` },
          ...(priceSummary.discount > 0 ? [{ label: 'Total Savings', value: `- ₹${formatCurrency(priceSummary.discount)}`, valueClass: 'text-emerald-700' }] : [])
        ]} />
        ...
        <div className="flex gap-2">
          <div className="relative flex-1">
            ...
            <input type="text" placeholder="Have a coupon?" value={couponCode} onChange={(e) => setCouponCode(e.target.value.toUpperCase())} className="w-full pl-9 pr-3 py-2.5 bg-gray-50 border border-gray-200 rounded-xl text-sm focus:outline-none focus:border-blue-500 transition" />
          </div>
          <button onClick={() => handleApplyCoupon?.(couponCode)} disabled={!couponCode} className="px-4 py-2 bg-gray-900 text-white text-xs font-bold rounded-xl hover:bg-black disabled:opacity-50 transition">APPLY</button>
        </div>
          ...
          <>
            <div className="bg-white rounded-3xl shadow-sm border border-gray-100 p-5">
              ...
              <div className="space-y-3">
                <input type="text" placeholder="Full Name" value={guestDetails.name} onChange={(e) => setGuestDetails({ ...guestDetails, name: e.target.value })} className="w-full px-4 py-3 bg-gray-50 border border-gray-200 rounded-xl text-sm" />
                <div className="grid grid-cols-2 gap-3">
                  <input type="tel" placeholder="Mobile Number" value={guestDetails.phone} onChange={(e) => setGuestDetails({ ...guestDetails, phone: e.target.value })} className="w-full px-4 py-3 bg-gray-50 border border-gray-200 rounded-xl text-sm" />
                  <input type="email" placeholder="Email (Optional)" value={guestDetails.email} onChange={(e) => setGuestDetails({ ...guestDetails, email: e.target.value })} className="w-full px-4 py-3 bg-gray-50 border border-gray-200 rounded-xl text-sm" />
                </div>
                {!guestFormValid && <p className="text-xs text-rose-600">Please enter valid name and mobile number.</p>}
              </div>
            </div>
            <button disabled={!guestFormValid || offlineBookingLoading || priceSummary.netPay <= 0} onClick={triggerOfflineBooking} className={`w-full py-4 bg-gradient-to-r ${roomsCount > 3 ? 'from-orange-500 to-orange-600' : 'from-blue-600 to-blue-700'} text-white font-bold text-lg rounded-2xl shadow-lg disabled:opacity-70 flex items-center justify-center gap-3`}>
              {offlineBookingLoading ? <Loader2 className="animate-spin" size={24} /> : <><span>{roomsCount > 3 ? 'Request Group Booking' : 'Book Now & Pay at Hotel'}</span><ChevronRight size={20} /></>}
            </button>
            {bookingStatus?.type === 'offline' && (
              <div className={`${bookingStatus.status === 'Pending' ? 'bg-orange-50 border-orange-200' : 'bg-emerald-50 border-emerald-200'} border rounded-2xl p-4 flex items-start gap-3`}>
                <CheckCircle2 size={24} className={bookingStatus.status === 'Pending' ? 'text-orange-600' : 'text-emerald-600'} />
                <div>
                  <h4 className="font-bold">{bookingStatus.status === 'Pending' ? 'Request Received' : 'Booking Confirmed!'}</h4>
  ...
  return (
    <div className="bg-gray-50 min-h-screen pb-24 lg:pb-20 font-sans compact-booknow">
      {/* Modals */}
      {showCalendar && <CalendarPicker checkIn={checkInDate} checkOut={checkOutDate} onCheckInChange={setCheckInDate} onCheckOutChange={setCheckOutDate} onClose={() => setShowCalendar(false)} />}
      {showPolicies && <PoliciesModal policies={hotel?.policies} onClose={() => setShowPolicies(false)} />}
      {showGallery && <GalleryModal images={galleryImages} startIndex={galleryIndex} title={hotelName} onClose={() => setShowGallery(false)} />}

      {/* Header */}
      <div className="bg-white border-b border-gray-200 shadow-sm sticky top-0 z-30">
        <div className="max-w-7xl mx-auto px-4 py-3 flex items-center gap-4">
          <button onClick={() => navigate(-1)} className="p-2 hover:bg-gray-100 rounded-full transition text-gray-600"><ChevronLeft size={22} /></button>
          <div className="min-w-0">
            <h1 className="text-base sm:text-lg font-bold text-gray-900 truncate">{hotelName}</h1>
            <p className="text-xs text-gray-500 flex items-center gap-1 truncate"><MapPin size={12} /> {hotel?.city}{hotel?.state ? `, ${hotel.state}` : ''}</p>
          </div>
        </div>
      </div>
          ...
          <div className="lg:col-span-2 space-y-6">
            ...
            <SectionCard >
              <div className="rounded-3xl overflow-hidden border border-gray-100 bg-gray-100">
                <button onClick={() => { setGalleryIndex(0); setShowGallery(true); }} className="block w-full">
                  <img src={galleryImages[0]} alt={hotelName} className="w-full h-[230px] sm:h-[320px] object-cover" />
                </button>
              </div>
              <div className="mt-3 flex gap-2 overflow-x-auto no-scrollbar">
                {galleryImages.slice(0, 8).map((img, i) => (
                  <button key={i} onClick={() => { setGalleryIndex(i); setShowGallery(true); }} className="shrink-0 w-16 h-16 rounded-xl overflow-hidden border border-gray-200 bg-gray-100">
                    <img src={img} alt="thumb" className="w-full h-full object-cover" />
                  </button>
                ))}
                {galleryImages.length > 8 && <button onClick={() => setShowGallery(true)} className="shrink-0 w-16 h-16 rounded-xl border border-gray-200 bg-white text-xs font-bold text-gray-700 flex items-center justify-center">+{galleryImages.length - 8}</button>}
              </div>
            </SectionCard>
            ...
            {/* Amenities */}
            <SectionCard title="Amenities" icon={<Sparkles size={20} className="text-yellow-500" />} right={amenitiesRemaining > 0 && <button onClick={() => setShowAllAmenities(!showAllAmenities)} className="text-blue-600 text-xs font-semibold hover:underline">{showAllAmenities ? 'Show less' : `+${amenitiesRemaining} more`}</button>}>
              <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
                {amenitiesToRender.map((amenity, idx) => (
                  <div key={idx} className="flex items-center gap-3 text-sm text-gray-700 border border-gray-100 bg-gray-50 rounded-2xl px-3 py-2">
                    <div className="w-8 h-8 rounded-xl bg-white border border-gray-200 flex items-center justify-center shrink-0">{getAmenityIcon(amenity)}</div>
            ...
            <SectionCard title="Rooms" icon={<Users size={20} className="text-indigo-600" />}>
              <div className="space-y-3">
                {rooms.map((room) => {
                  ...
                  return (
                    <button key={room.id} onClick={() => canSelect && setSelectedRoomId(room.id)} disabled={!canSelect} className={`w-full text-left rounded-2xl border p-4 transition ${isSelected ? 'border-blue-600 bg-blue-50/40' : 'border-gray-100 bg-white'} ${!canSelect ? 'opacity-60' : 'hover:bg-gray-50'}`}>
                      <div className="flex gap-3">
                        ...
                        <div className="flex-1 min-w-0">
                          <div className="flex items-center gap-2">
                            ...
                            {isSelected && <span className="text-[10px] font-bold px-2 py-1 rounded-lg bg-blue-600 text-white">Selected</span>}
                          </div>
                          <div className="text-xs text-gray-500">{room.area} • Max 3 Guests</div>
                          <div className="mt-2 flex items-center justify-between">
                            ...
                            <span className={`text-xs font-bold px-2 py-1 rounded-lg ${room.isAvailable ? 'bg-emerald-50 border-emerald-100 text-emerald-700' : 'bg-gray-50 border-gray-200 text-gray-700'} border`}>
                    ...
                    return (
                      <div key={idx} className="rounded-2xl border border-gray-100 p-4 bg-white">
                        <div className="flex items-start justify-between gap-3">
                          <div><div className="font-bold text-gray-900 text-sm">{food?.name || 'Meal'}</div><div className="text-xs text-gray-500">₹{formatCurrency(food?.price)} per item</div></div>
                          <div className="flex items-center gap-2 rounded-xl border border-gray-200 bg-white px-2 py-1">
                            <button onClick={() => upsertFood(food, qty - 1)} className="w-8 h-8 rounded-lg hover:bg-gray-50 flex items-center justify-center text-gray-600"><Minus size={14} /></button>
                            <span className="w-6 text-center font-bold text-gray-900">{qty}</span>
                            <button onClick={() => upsertFood(food, qty + 1)} className="w-8 h-8 rounded-lg hover:bg-gray-50 flex items-center justify-center text-gray-600"><Plus size={14} /></button>
                          </div>
                        </div>
                        {qty > 0 && <div className="mt-3 flex items-center justify-between text-xs text-gray-600"><span>Subtotal</span><span className="font-bold text-gray-900">₹{formatCurrency(parseNumber(food?.price, 0) * qty)}</span></div>}
                      </div>
                    );
                  })}
                </div>
            ...
            {/* Policies */}
            <SectionCard title="Policies" icon={<Info size={18} className="text-blue-500" />} right={<button onClick={() => setShowPolicies(true)} className="text-blue-600 text-xs font-semibold hover:underline">View all</button>}>
              {policyHighlights.length ? (
                <div className="flex flex-wrap gap-2">
                  {policyHighlights.map((p, idx) => {
                    const badge = badgeForPolicy(p.val);
                    return <div key={idx} className="flex items-center gap-2 px-2.5 py-1 rounded-lg border border-gray-100 bg-gray-50"><span className="text-[11px] text-gray-600">{p.label}</span><span className={`text-[11px] font-bold px-2 py-0.5 rounded-md border ${badge.cls}`}>{badge.text}</span></div>;
                  })}
                </div>
                    ...
                    return (
                      <div key={i} className="rounded-2xl border border-gray-100 bg-white p-4">
                        <div className="flex items-start justify-between gap-3">
                          ...
                          {rating > 0 && <span className="inline-flex items-center gap-1 px-2 py-1 rounded-lg bg-emerald-50 border border-emerald-100 text-emerald-700 text-xs font-bold"><Star size={12} fill="currentColor" />{rating}</span>}
                        </div>
      ...
      <div className="lg:hidden fixed bottom-0 left-0 right-0 z-50 border-t border-gray-200 bg-white/95 backdrop-blur">
        <div className="max-w-7xl mx-auto px-4 py-3 flex items-center gap-3">
          <button onClick={() => setShowBookingSheet(true)} className="flex-1 text-left rounded-2xl border border-gray-200 bg-white px-4 py-2">
            <div className="text-[11px] uppercase font-bold text-gray-400">Total</div>
            <div className="text-base font-extrabold text-gray-900">₹{formatCurrency(priceSummary.netPay)}</div>
          </button>
          <button disabled={!guestFormValid || offlineBookingLoading} onClick={() => setShowBookingSheet(true)} className="px-5 py-3 rounded-2xl bg-blue-600 text-white font-bold disabled:opacity-60">Book</button>
        </div>
      </div>
      ...
      {showBookingSheet && (
        <div className="fixed inset-0 z-[140] bg-black/40 backdrop-blur-[2px]">
          <div className="absolute inset-x-0 bottom-0 bg-white rounded-t-3xl shadow-2xl max-h-[90vh] overflow-y-auto">
            ...
            <button onClick={() => setShowBookingSheet(false)} className="absolute top-4 right-4 z-10 p-2 rounded-full bg-white/90 hover:bg-white shadow-md transition">
            ...
            <div className="bg-gradient-to-r from-blue-600 to-blue-700 px-5 pt-6 pb-4 text-white rounded-t-3xl">
              ...
              <p className="text-sm text-blue-100 mt-1">{nights} Night{nights > 1 ? 's' : ''} at {hotelName}</p>
            </div>
            ...
            <div className="px-5 py-4 space-y-4">
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <button onClick={() => setShowCalendar(true)} className="w-full text-left p-3 bg-gray-50 rounded-xl hover:bg-gray-100 transition border border-gray-200">
                    <div className="text-sm font-bold text-gray-900">{formatDateShort(checkInDate)} - {formatDateShort(checkOutDate)}</div>
                  </button>
                </div>
                <div>
                  <div className="text-xs uppercase font-bold text-gray-500 mb-2 flex items-center gap-1">
                  ...
                  <button onClick={() => setShowRoomsPopup(!showRoomsPopup)} className="w-full text-left p-3 bg-gray-50 rounded-xl hover:bg-gray-100 transition border border-gray-200">
                    <div className="text-sm font-bold text-gray-900">{roomsCount}R, {guestsCount}G</div>
                  </button>
                </div>
              </div>
              ...
              <div className="bg-white rounded-xl border border-gray-200 p-4 space-y-2">
                <div className="flex justify-between text-sm">
                  <span className="text-gray-600">Room Price ({nights} night{nights > 1 ? 's' : ''})</span>
                  <span className="font-bold text-gray-900">₹{formatCurrency(priceSummary.roomTotal)}</span>
                </div>
                {priceSummary.gst > 0 && (
                  <div className="flex justify-between text-sm">
                    <span className="text-gray-600">Taxes & Fees</span>
                    <span className="font-bold text-gray-900">₹{formatCurrency(priceSummary.gst)}</span>
                  </div>
                )}
                <div className="border-t border-gray-200 pt-2 mt-2 flex justify-between items-center">
                  <span className="font-bold text-gray-900">Total Payable</span>
                  <span className="text-xl font-extrabold text-blue-600">₹{formatCurrency(priceSummary.netPay)}</span>
                </div>
              </div>
              ...
              <div className="flex items-center gap-2">
                <div className="flex-1 relative">
                  ...
                  <input type="text" placeholder="Enter coupon code" value={couponCode} onChange={(e) => setCouponCode(e.target.value)} className="w-full pl-10 pr-4 py-3 bg-gray-50 border border-gray-200 rounded-xl text-sm" />
                </div>
                <button onClick={applyCoupon} disabled={!couponCode.trim()} className="px-5 py-3 bg-blue-600 text-white font-bold rounded-xl disabled:opacity-50 disabled:cursor-not-allowed hover:bg-blue-700 transition">
                  Apply
                </button>
              </div>
              {couponMessage && <p className={`text-xs ${couponMessage.includes('applied') ? 'text-emerald-600' : 'text-rose-600'}`}>{couponMessage}</p>}
              ...
              <div className="bg-gray-50 rounded-xl border border-gray-200 p-4">
                ...
                <div className="space-y-3">
                  <input type="text" placeholder="Full Name" value={guestDetails.name} onChange={(e) => setGuestDetails({ ...guestDetails, name: e.target.value })} className="w-full px-4 py-3 bg-white border border-gray-200 rounded-xl text-sm focus:border-blue-500 focus:outline-none" />
                  <div className="grid grid-cols-2 gap-3">
                    <input type="tel" placeholder="Mobile Number" value={guestDetails.phone} onChange={(e) => setGuestDetails({ ...guestDetails, phone: e.target.value })} className="w-full px-4 py-3 bg-white border border-gray-200 rounded-xl text-sm focus:border-blue-500 focus:outline-none" />
                    <input type="email" placeholder="Email" value={guestDetails.email} onChange={(e) => setGuestDetails({ ...guestDetails, email: e.target.value })} className="w-full px-4 py-3 bg-white border border-gray-200 rounded-xl text-sm focus:border-blue-500 focus:outline-none" />
                  </div>
                  {!guestFormValid && <p className="text-xs text-rose-600">Please enter valid name and mobile number.</p>}
                </div>
              </div>
              ...
              {/* Book Now Button */}
              <button disabled={!guestFormValid || offlineBookingLoading} onClick={triggerOfflineBooking} className={`w-full py-4 bg-gradient-to-r ${roomsCount > 3 ? 'from-orange-500 to-orange-600' : 'from-blue-600 to-blue-700'} text-white font-bold text-lg rounded-xl shadow-lg disabled:opacity-70 flex items-center justify-center gap-3`}>
                {offlineBookingLoading ? <Loader2 className="animate-spin" size={24} /> : <><span>{roomsCount > 3 ? 'Request Group Booking' : 'Book Now & Pay at Hotel'}</span><ChevronRight size={20} /></>}
              </button>

              {/* Booking Status */}
              {bookingStatus?.type === 'offline' && (
                <div className={`${bookingStatus.status === 'Pending' ? 'bg-orange-50 border-orange-200' : 'bg-emerald-50 border-emerald-200'} border rounded-xl p-4 flex items-start gap-3`}>
                  <CheckCircle2 size={24} className={bookingStatus.status === 'Pending' ? 'text-orange-600' : 'text-emerald-600'} />
                  <div>
                    <h4 className="font-bold">{bookingStatus.status === 'Pending' ? 'Request Received' : 'Booking Confirmed!'}</h4>
                    <p className="text-sm mt-1">Reference: {bookingStatus.reference}</p>
