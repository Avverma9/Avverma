...
const HotelPageContent = () => {
  ...
  const amenityItems = useMemo(
    () => Object.entries(amenityIcons).map(([name, icon]) => ({ name, icon })),
    []
  );
  ...
  const fetchData = useCallback(
    async (pageNum) => {
      ...
      try {
        ...
        setHotelData((prevData) => (isFirstPage ? newData : [...prevData, ...newData]));
        ...
        if (newData.length > 0) {
          const maxRoomPrice = Math.max(
            ...newData.flatMap((h) =>
              h.rooms?.map((r) => parseFloat(r.price)) || [0]
            )
          );
          if (maxRoomPrice) {
  ...
  useEffect(() => {
    ...
    // Always create a fresh observer to capture current state
  ...
  }, [hasMore, isLoading, hotelData.length]);
          ...
          <Box>
            ...
            <Slider
              ...
              sx={{
                ...
                "& .MuiSlider-track": {
                  background:
                    "linear-gradient(90deg, #4f46e5, #7c3aed) !important",
                },
            ...
            <div className="p-3 space-y-2">
              ...
              <div className="flex flex-wrap gap-1">
                {allAmenities.slice(0, 3).map((amenity, i) => (
                  <span
                    key={i}
                    className="px-2 py-0.5 bg-indigo-100 text-indigo-600 text-[0.65rem] h-5 font-semibold rounded-full"
                  >
                    {amenity}
                  </span>
                ))}
              ...
              <button
                ...
                className="w-full bg-gradient-to-br from-indigo-500 to-purple-600 text-white font-bold py-2.5 rounded-lg normal-case text-sm shadow-lg shadow-indigo-500/30 hover:from-purple-600 hover:to-indigo-500 transition-all"
              >
                View Details
              </button>
            </div>
          ...
          <div className="flex">
            ...
            <div className="w-1/4 border-l border-indigo-100">
              <div className="p-6 h-full flex flex-col justify-center items-end text-right bg-gradient-to-br from-[#f8f9ff] to-white">
                ...
                <button
                  onClick={() => handleBuy(hotel.hotelId)}
                  className="bg-gradient-to-br from-indigo-500 to-purple-600 text-white font-bold py-3 px-8 rounded-xl normal-case text-sm shadow-lg shadow-indigo-500/30 hover:from-purple-600 hover:to-indigo-500 hover:-translate-y-0.5 hover:shadow-xl transition-all"
                >
                  View Details
                </button>
              </div>
  ...
  return (
    ...
    >
      ...
      {/* Content Area with Fixed Sidebar and Scrollable Hotel List */}
      <Box
        sx={{
          display: "flex",
      ...
      >
        ...
        >
          ...
          <Box sx={{ p: 1, overflowY: "auto" }}>{renderFilters()}</Box>
        </Drawer>

        {/* Scrollable Hotel Content Area */}
        <Box
          sx={{
            flex: 1,
            overflowY: { md: "auto" },
