  ...
  return (
    <>
      ...
      <header className="fixed top-0 left-0 right-0 z-50 p-4">
        <div
          className={`mx-auto max-w-7xl transition-all duration-500 ease-out transform ${
            headerScrolled
              ? "bg-white/10 backdrop-blur-2xl border border-white/20 shadow-2xl rounded-2xl" // Scrolled state styles
              : "bg-white/5 backdrop-blur-xl border border-white/10 shadow-lg rounded-2xl" // Initial state styles
          }`}
        >
              ...
              <div className="relative hidden lg:block">
                ...
                {profileMenuOpen && (
                  <div 
                    ...
                    className="w-48 bg-white/10 backdrop-blur-2xl rounded-xl shadow-2xl py-2 z-10 border border-white/20"
                  >
      ...
      {/* Mobile Menu Sidebar */}
      <aside
        className={`fixed top-0 left-0 h-full w-full max-w-xs bg-white/10 backdrop-blur-2xl shadow-2xl transform transition-all duration-300 ease-in-out z-50 border-r border-white/20 ${
          ...
          <ul className="space-y-2">
            {navLinks.map((link) => (
              <li key={link.text}>
                <button
                  onClick={() => handleRedirect(link.path)}
                  className={`flex items-center gap-4 p-3 rounded-xl w-full text-left font-medium transition-all duration-200 ${
              ...
              {profileLinks.map((link) => (
                <li key={link.text}>
                  <button
                    onClick={() => handleRedirect(link.path)}
                    className={`flex items-center gap-4 p-3 rounded-xl w-full text-left font-medium transition-all duration-200 ${
                      location.pathname.startsWith(link.path) ? "bg-white/20 shadow-sm backdrop-blur-sm border border-white/10" : ""
                    } ${
                      link.text === 'Logout' 
                        ? 'text-red-500 hover:bg-red-500/10' 
